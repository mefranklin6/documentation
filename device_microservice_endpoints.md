...
GET returns `{"occupancy_detected": "false"}`, or an error.
...
The endpoint `"calendarpasscode"` only uses `start_time` and `stop_time`. It could be extended to recognize different JSON responses and parse those too (pull requests are welcome).
